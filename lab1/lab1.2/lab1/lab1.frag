...
void main(void)
{
	if (displayGPUversion == 1)
	{
		...
		float stripes = mod(round(texCoord.y*160),8); // every 10 is black
		float isEven = mod(round((texCoord.y) *20 +0.5),2);

		float verticalStripesEven = mod(round(texCoord.x*80),8) * isEven;
		float verticalStripesOdd = mod(round(texCoord.x*80 + 20),8) * (isEven + 1 - isEven * 2);

		float bricks = verticalStripesOdd + verticalStripesEven;
		float mask = clamp(bricks * stripes, 0, 1);
		//printf("%f\n", bricks * stripes);

        if (mask < 0.5) {
            out_Color = vec4(1,0.8,0.8,1);
        } else {
            out_Color = vec4(1,0.3,0.3,1);
        }
        ...
        //out_Color = vec4(cos(radius * ringDensity*10)/ 2.0 + 0.5, 0.5, sin(radius * ringDensity*2)/ 2.0 + 0.5, 1.0);
        //out_Color = vec4(1,0.3,0.3,1)*mask;
