...
{
	int x, y;
	float fx, fy, fxo, fyo, fkTextureSize;
	fkTextureSize = kTextureSize;
	char val;
	for (x = 0; x < kTextureSize; x++)
	for (y = 0; y < kTextureSize; y++)
	{

	    fx = x/fkTextureSize * 15;
	    fy = y/fkTextureSize * 15;
	    //printf("%f\n", fx);
		ptex[x][y][0] = noise2(fx, fy)*255;
		ptex[x][y][1] = noise2(fx, fy)*255;
		ptex[x][y][2] = noise2(fx, fy)*255;
